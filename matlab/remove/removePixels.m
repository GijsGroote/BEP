function [mov, J_data] = removePixels(path, percentage)
...
tic;
k = 0;
while k < 10  %hasFrame(v) %the amount of frames. Use hasFrame(v) to get all of the frames in the video
...
for i = 1:k
    ...
    J_zeros = uint8(zeros(total_pixels,1,3));
    rand_unique_pixels = randperm(total_pixels,round((total_pixels)*(1-p)));
    ...
    J_zeros(rand_unique_pixels,:,:) = 1;
    
    modified_frame = reshape(empty_frame,[vidHeight,vidWidth,3]);
    mov(i).cdata = modified_frame;
    
    J_frame = reshape(J_zeros,[vidHeight,vidWidth,3]);
    J_data(i).cdata = J_frame;
